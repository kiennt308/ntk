...
### Q1: Giải thích sự khác biệt bản chất giữa mô hình Declarative (Khai báo) của Terraform và Imperative (Mệnh lệnh) của Ansible / Bash Script?
- **Trả lời**: 
...
### Q2: Quá trình "Two-Phase Execution" (Khởi tạo 2 giai đoạn) của Terraform diễn ra như thế nào?
- **Trả lời**: Gồm 2 giai đoạn tách biệt:
...
### Q6: `terraform init` thực hiện những tác vụ gì dưới nền tảng?
- **Trả lời**: Khi chạy `terraform init`, Terraform thực hiện 4 tác vụ chính:
...
### Q7: Sự khác nhau giữa `terraform.tfvars`, `*.auto.tfvars`, và cờ `-var` là gì?
- **Trả lời**:
...
flowchart TD
    ...
    style STATE_OP fill:#f9f9f9,stroke:#333,stroke-width:2px
    style MV fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
    style RM fill:#fff3e0,stroke:#f57c00,stroke-width:2px
    style IMP fill:#d4edda,stroke:#28a745,stroke-width:2px
    style UNLOCK fill:#ffebee,stroke:#c62828,stroke-width:2px


```

### Q8: Trình bày cấu trúc JSON bên trong của một tệp State file phiên bản 4 (`"version": 4`)?
- **Trả lời**: Tệp State JSON v4 bao gồm các trường cốt lõi:
...
### Q9: Tại sao việc sử dụng `count` để lặp danh sách tài nguyên lại tiềm ẩn rủi ro "Index Shifting" và cách khắc phục?
- **Trả lời**: 
...
### Q10: Khối `moved` (Terraform 1.1+) giải quyết triệt để bài toán Refactoring như thế nào so với lệnh `terraform state mv`?
- **Trả lời**: Lệnh `terraform state mv` là lệnh mệnh lệnh (Imperative CLI), đòi hỏi kỹ sư phải gõ thủ công trên terminal, dễ gõ sai tên, không thể lưu vết trên Git và không có cơ chế Peer Review qua Pull Request. Khối `moved { from = ... to = ... }` biến việc chuyển dịch State thành **Khai báo mã nguồn (Declarative)**: có thể commit vào Git, được CI/CD hiển thị trong `terraform plan` dưới dạng *"Resource has moved (0 to destroy)"*, và áp dụng đồng loạt an toàn trên mọi môi trường.

### Q11: Làm thế nào để xóa một tài nguyên ra khỏi quyền quản lý của Terraform mà KHÔNG XÓA tài nguyên thực tế trên Cloud?
- **Trả lời**: 
...
### Q12: Giải thích sự khác biệt giữa `terraform import` qua CLI và khối `import` khai báo trong Terraform 1.5+?
- **Trả lời**: Lệnh CLI `terraform import` chỉ ghi đè vào State và yêu cầu kỹ sư phải tự viết code HCL bằng tay. Khối `import { to = ... id = ... }` trong Terraform 1.5+ cho phép đưa vào file `.tf`, lưu vào Git, và hỗ trợ cờ **`-generate-config-out=generated.tf`** để Terraform tự động viết hoàn chỉnh mã nguồn HCL tương ứng với tài nguyên thực tế trên Cloud.

### Q13: State Drift là gì và Terraform xử lý Drift như thế nào?
- **Trả lời**: State Drift là hiện tượng bất đồng bộ giữa cấu hình khai báo trong code HCL, dữ liệu ghi trong State file và trạng thái thực tế của tài nguyên trên Cloud (thường do ai đó sửa trực tiếp trên Web Console). Khi chạy `terraform plan`, Terraform thực hiện **State Refresh**: gửi API tới Cloud để cập nhật State file, sau đó so sánh Desired State (code) với Refreshed State để đề xuất kế hoạch đưa tài nguyên thực tế trở lại đúng như trong code (Reconciliation).

---

## 3. Cấp Độ 3: Thiết Kế Module, Vòng Đời & Meta-Arguments (Senior Level)

### Q14: Giải thích cơ chế hoạt động của `create_before_destroy = true` và cạm bẫy Name Collision?
- **Trả lời**: 
...
### Q15: `prevent_destroy = true` bảo vệ tài nguyên như thế nào và làm sao để decommission một tài nguyên có gắn cờ này?
- **Trả lời**: Khi bất kỳ kế hoạch nào (kể cả lệnh `terraform destroy` hoặc một thay đổi code gây force-replacement) có chứa hành động xóa tài nguyên có `prevent_destroy = true`, Terraform Core sẽ dừng lại ngay trong bước Plan và báo lỗi Fatal Error. Để decommission, kỹ sư bắt buộc phải sửa tường minh `prevent_destroy = false` trong code, chạy `terraform apply` để nạp vào State, sau đó mới được phép xóa.

### Q16: `replace_triggered_by` trong khối `lifecycle` (Terraform 1.2+) khác gì so với `depends_on`?
- **Trả lời**: `depends_on` chỉ quy định thứ tự khởi tạo (Tài nguyên A phải tạo trước B). `replace_triggered_by` định nghĩa quan hệ kích hoạt tái tạo: Khi tài nguyên được chỉ định (hoặc một `terraform_data` hash) bị thay đổi hoặc recreate, tài nguyên hiện tại bắt buộc phải bị **Destroy & Recreate theo**.

### Q17: Tại sao tài nguyên `terraform_data` (Terraform 1.4+) lại thay thế hoàn toàn `null_resource`?
- **Trả lời**: `null_resource` đòi hỏi phải tải thêm một provider bên ngoài `hashicorp/null` và chỉ hỗ trợ trigger dạng `map(string)`. `terraform_data` là tài nguyên tích hợp sẵn trong Terraform Core (không cần tải provider), hỗ trợ lưu trữ kiểu dữ liệu tùy ý (`input`/`output`), và hỗ trợ `triggers_replace` cho mọi Complex Types.

### Q18: Làm thế nào để truyền nhiều Provider Instances khác nhau vào bên trong Child Module một cách bài bản?
- **Trả lời**: Trong Child Module, khai báo danh sách bí danh trong khối `required_providers`:
  ```hcl
  terraform {
    required_providers {
      aws = {
...
### Q20: Tại sao cờ `sensitive = true` không ngăn chặn được việc rò rỉ mật khẩu trong Terraform State?
- **Trả lời**: Cờ `sensitive = true` chỉ là một tính năng định dạng ở tầng giao diện hiển thị (CLI/UI), nhằm ngăn chặn in chuỗi mật khẩu ra màn hình terminal hoặc nhật ký CI/CD. Tuy nhiên, trong tệp `terraform.tfstate`, **toàn bộ dữ liệu mật khẩu vẫn được ghi dưới dạng văn bản thuần (Plaintext JSON)**. Bất kỳ ai có quyền đọc file State đều có thể lấy được mật khẩu.

### Q21: Trình bày cơ chế hoạt động của tính năng Ephemeral Values trong Terraform 1.10+?
- **Trả lời**: Biến số hoặc tài nguyên được đánh dấu `ephemeral = true` chỉ tồn tại tạm thời trong bộ nhớ RAM trong suốt quá trình Plan và Apply để gửi tới Cloud API. Sau khi hoàn tất, Terraform Core **chủ động loại bỏ hoàn toàn giá trị này khỏi tệp `terraform.tfstate`**, giúp State file hoàn toàn sạch bóng các Plaintext Secrets.

### Q22: Cơ chế xác thực Keyless OIDC (OpenID Connect) giữa GitHub Actions và AWS hoạt động như thế nào?
- **Trả lời**: GitHub Actions Runner gửi yêu cầu tới GitHub OIDC Provider để nhận một JWT Token có chữ ký số chứa các Claims (tên repo, branch, commit). Runner gửi JWT này tới AWS STS thông qua API `sts:AssumeRoleWithWebIdentity`. AWS STS xác thực chữ ký của GitHub, đối chiếu Trust Policy của IAM Role (khớp chính xác `repo:org/repo:ref:refs/heads/main`), và cấp ngược lại một bộ thông tin xác thực tạm thời (Temporary Credentials có hạn 1 giờ). Quá trình này **loại bỏ 100% việc lưu trữ IAM Access Keys tĩnh** trên GitHub Secrets.

### Q23: Tại sao trong CI/CD Pipeline bắt buộc phải lưu file kế hoạch nhị phân `terraform plan -out=tfplan.binary`?
- **Trả lời**: Để đảm bảo **tính tất định (Determinism)**. File plan nhị phân là một snapshot cố định chứa chính xác những gì đã được thẩm định và phê duyệt trong Pull Request. Khi chạy `terraform apply tfplan.binary`, Terraform chỉ thực thi đúng những gì trong file đó, ngăn chặn nguy cơ ai đó thay đổi cấu hình Cloud ngầm giữa thời điểm Plan và Apply.

### Q24: Phân tích sự khác biệt giữa Policy as Code bằng Open Policy Agent (OPA/Rego) và HashiCorp Sentinel?
- **Trả lời**: 
...
flowchart LR
    ...
    style INCIDENT fill:#ff0000,stroke:#ff0000,stroke-width:2px
    style UNLOCK fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
    style RESTORE fill:#d4edda,stroke:#28a745,stroke-width:2px


```

### Q25: Khi nhận được lỗi "Error acquiring the state lock: ConditionalCheckFailedException", bạn xử lý như thế nào theo đúng quy trình SRE?
- **Trả lời**: 
...
### Q26: Nếu máy chủ CI Runner bị mất điện giữa chừng khi đang chạy `terraform apply`, dẫn đến việc tài nguyên đã được tạo trên Cloud nhưng chưa kịp ghi vào State (Orphaned Resource), bạn khắc phục ra sao?
- **Trả lời**: 
...
### Q27: Tại sao khi chỉnh sửa thủ công một tệp State JSON bị hỏng, việc tăng số `"serial"` là bắt buộc?
- **Trả lời**: Terraform Remote Backend sử dụng trường `"serial"` như một cơ chế kiểm tra tính mới của dữ liệu (Optimistic Concurrency Control). Nếu bạn đẩy lên một file State có `serial` nhỏ hơn hoặc bằng số `serial` hiện tại mà Backend đang lưu trữ, Backend sẽ từ chối nạp với lỗi `State serial number is older than current remote state`. Tăng `serial` lên +1 đảm bảo Backend chấp nhận file khôi phục mới.

### Q28: Làm thế nào để xử lý lỗi "Cycle: module.a, module.b" (Vòng lặp đồ thị phụ thuộc) khi sử dụng `create_before_destroy`?
- **Trả lời**: Lỗi Cycle xảy ra khi Tài nguyên A (có CBD=true) phụ thuộc vào Tài nguyên B (có CBD=false). Khi sửa B, Terraform cố tạo A trước, nhưng A cần B mới tạo xong, trong khi B lại phải đợi xóa A cũ trước. Cách khắc phục: **Lan truyền CBD (CBD Propagation)** bằng cách đặt `create_before_destroy = true` cho cả Tài nguyên B và tất cả các tài nguyên phụ thuộc liên quan.

### Q29: Một kỹ sư vô tình chạy `terraform state rm` trên toàn bộ cơ sở dữ liệu Production RDS. Hạ tầng có bị xóa không và làm sao để cứu hộ?
- **Trả lời**: 
...
### Q30: Phân tích chiến lược kiểm soát "Blast Radius" khi quy mô hạ tầng đạt hơn 5,000 tài nguyên đám mây?
- **Trả lời**: 
...
### Q31: Terragrunt hiện thực hóa triết lý DRY (Don't Repeat Yourself) như thế nào so với Terraform thuần túy?
- **Trả lời**: Terragrunt cung cấp:
...
### Q32: Khi nào một tập đoàn nên lựa chọn CDKTF thay vì HCL truyền thống?
- **Trả lời**: Khi đội ngũ phát triển ứng dụng (Software Developers) chiếm đa số và muốn tự vận hành hạ tầng bằng TypeScript/Python, khi hạ tầng có các thuật toán phân bổ tài nguyên phức tạp cần cấu trúc dữ liệu hướng đối tượng (OOP), hoặc khi doanh nghiệp muốn tích hợp các bộ framework Unit Test tiêu chuẩn như Jest/PyTest vào quy trình kiểm thử hạ tầng.

### Q33: Làm thế nào để quản trị vấn đề Cloud API Throttling (Rate Limiting) khi chạy `terraform plan` trên hàng nghìn tài nguyên?
- **Trả lời**: 
...
### Q38: Trong mô hình GitOps cho Terraform, công cụ Atlantis hoạt động như thế nào?
- **Trả lời**: Atlantis là một server lắng nghe Webhook từ Git. Lập trình viên tương tác với Terraform thông qua comment trong Pull Request (ví dụ: `atlantis plan`, `atlantis apply`). Atlantis tự động khóa nhánh (Branch Lock), chạy lệnh trên môi trường cô lập, comment kết quả vào PR và chỉ cho phép apply khi PR đã được Approve đầy đủ.

### Q39: Làm thế nào để quản trị hàng trăm Kubernetes Clusters xuyên qua nhiều Cloud Providers bằng Terraform?
...
### Q40: Lời khuyên quan trọng nhất của bạn dành cho một Cloud Engineer bắt đầu thiết kế hệ thống IaC cho doanh nghiệp là gì?
- **Trả lời**: **"Start Modular, Keep States Small, and Automate Governance Early"** — Hãy thiết kế module hóa ngay từ ngày đầu, chia nhỏ State files để cô lập Blast Radius dưới 150 tài nguyên/state, và thiết lập CI/CD Pipeline không dùng Static Keys (OIDC) kết hợp với Policy as Code (OPA/Rego) trước khi hạ tầng phình to vượt tầm kiểm soát.

### Q41: Tại sao nên sử dụng Data Source `aws_ami` với bộ lọc `most_recent = true` và `owners` thay vì hardcode AMI ID cố định?
- **Trả lời**: Vì AMI ID thay đổi theo từng Region và được cập nhật các bản vá bảo mật định kỳ hàng tháng. Việc hardcode ID sẽ làm mã nguồn không thể tái sử dụng trên nhiều Region và nhanh chóng bị lỗi thời. Sử dụng Data Source với bộ lọc `name`, `owner` và `most_recent = true` giúp code luôn tự động lấy phiên bản Golden Image mới nhất đã qua kiểm duyệt bảo mật.

### Q42: Trong kịch bản Disaster Recovery Multi-Region, làm thế nào để đồng bộ hóa Route53 DNS Failover tự động với Terraform?
- **Trả lời**: Sử dụng `aws_route53_record` với chính sách định tuyến `Failover Routing Policy`. Khởi tạo 2 records: Primary Record trỏ về Load Balancer tại Region chính (Singapore) kèm theo `health_check_id`, và Secondary Record trỏ về Load Balancer tại DR Region (Tokyo). Khi Health Check của vùng chính thất bại, Route53 sẽ tự động chuyển 100% traffic sang vùng dự phòng.

### Q43: Sự khác biệt giữa `terraform plan -target` và việc chạy plan bình thường là gì và tại sao việc lạm dụng `-target` lại nguy hiểm?
- **Trả lời**: 
...
### Q44: Làm thế nào để kiểm tra tính hợp lệ của cú pháp HCL trong toàn bộ thư mục lồng nhau mà không cần cài đặt Terraform trên máy dev?
- **Trả lời**: Sử dụng Docker Container chính thức của HashiCorp:
  ```bash
  docker run --rm -v $(pwd):/workspace -w /workspace hashicorp/terraform:latest fmt -check -recursive
  ```

### Q45: Giải thích vai trò của `precondition` và `postcondition` trong khối `lifecycle`?
- **Trả lời**:
...
### Q46: Khi gặp lỗi "Error: Invalid count argument" khi truyền một computed value từ data source vào count, nguyên nhân gốc rễ là gì?
- **Trả lời**: Trong quá trình dựng đồ thị Graph ban đầu, Terraform cần biết chính xác số lượng tài nguyên cần tạo để vẽ các Node. Nếu giá trị `count` phụ thuộc vào một thuộc tính chỉ được tính toán sau khi Cloud API phản hồi (Computed Value từ data source hoặc resource khác), Terraform không thể xác định số lượng Node và sẽ báo lỗi. Khắc phục: Phải sử dụng giá trị đã biết trước (known static value) hoặc cấu trúc `for_each` với tập hợp keys tĩnh.

### Q47: Làm thế nào để cấu hình AWS S3 Bucket với Object Lock để chống lại tấn công Ransomware xóa State file?
- **Trả lời**: Khởi tạo tài nguyên `aws_s3_bucket_object_lock_configuration` với chế độ `COMPLIANCE` mode và thời hạn `retention_period` (ví dụ: 30 ngày). Khi kích hoạt, không bất kỳ người dùng IAM nào (kể cả Root Account) có thể ghi đè hoặc xóa các phiên bản State trong khoảng thời gian retention.

### Q48: Làm thế nào để tối ưu hóa chi phí NAT Gateway trong kiến trúc Multi-AZ VPC bằng Terraform?
- **Trả lời**:
...
### Q49: Tại sao việc sử dụng `jsonencode()` trong HCL lại được khuyến nghị hơn việc sử dụng Heredoc string `<<EOF` khi viết IAM Policy?
- **Trả lời**: Vì `jsonencode()` đảm bảo tính hợp lệ của cú pháp JSON (chống lỗi syntax thiếu dấu phẩy, thiếu ngoặc nhọn), hỗ trợ ép kiểu dữ liệu HCL chính xác, và tự động bỏ qua các trường null, giúp code sạch và dễ bảo trì hơn rất nhiều so với chuỗi văn bản thuần túy.

### Q50: Nguyên lý "Immutable Infrastructure" (Hạ tầng bất biến) được Terraform hiện thực hóa như thế nào?
- **Trả lời**: Thay vì đăng nhập SSH vào máy chủ đang chạy để vá lỗi và cập nhật phần mềm (Mutable Infrastructure), Terraform kết hợp với Packer để đóng gói AMI mới và thực hiện quy trình **Destroy & Recreate (hoặc Create-before-Destroy Rolling Update)**. Mọi phiên bản máy chủ mới đều bắt đầu từ trạng thái sạch (Clean State), giúp loại bỏ hoàn toàn hiện tượng Configuration Drift.

### Q51: Làm thế nào để mã hóa toàn bộ dữ liệu State file bằng AWS KMS Customer Managed Key thay vì dùng khóa mặc định SSE-S3?
- **Trả lời**: Trong cấu hình Backend S3, chỉ định thuộc tính `kms_key_id`:
  ```hcl
  terraform {
    backend "s3" {
...
### Q52: Khi nào bạn nên sử dụng `terraform_remote_state` data source và khi nào nên tránh tuyệt đối?
- **Trả lời**:
...
### Q53: Làm thế nào để kiểm tra sự tồn tại của một file cục bộ trước khi nạp vào Terraform resource?
- **Trả lời**: Sử dụng hàm `fileexists(path)` kết hợp với `can()` hoặc `precondition`:
  ```hcl
  lifecycle {
    precondition {
...
### Q54: Sự khác biệt giữa `toset()` và `tolist()` trong HCL là gì?
- **Trả lời**:
...
### Q55: Trong trường hợp bạn có 100 EC2 instances được tạo bằng `for_each`, làm thế nào để chỉ áp dụng thay đổi cho đúng 1 instance cụ thể?
- **Trả lời**: Sử dụng cờ `-target` với cú pháp nhắm chính xác vào key:
  ```bash
