...
- **Cơ chế liên kết:** Deployment tự động tính toán mã băm từ cấu hình `spec.template` và gắn nhãn **`pod-template-hash`** ngẫu nhiên lên ReplicaSet và các Pod. ReplicaSet dùng `spec.selector` khớp với nhãn `pod-template-hash` này để quản lý tập Pod thuộc phiên bản đó.

**Tiêu chí chấm:**
- **0đ:** Bảo Deployment trực tiếp tạo và quản lý Pod.
- **1đ:** Nêu được 3 tầng nhưng không giải thích được cơ chế liên kết nhãn `pod-template-hash` (dính trần 1đ).
- **2đ:** Phân tích chuẩn xác mối quan hệ 3 tầng và cơ chế liên kết qua nhãn `pod-template-hash`.
