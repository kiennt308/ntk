...
tldr:
  - "Nắm vững nguyên lý nền tảng và tư duy cốt lõi về Cấu Hình Audit Logging Nâng Cao: Thiết Kế Audit Policy, Ghi Nhận Sự Kiện & Truy Vết Tấn Công An Ninh Mạng."
...
Ghi lại 100% các hành vi tương tác với API Server để phục vụ quá trình <b style="color: var(--accent-primary);">điều tra sự cố bảo mật (Security Incident Response)</b>.
</div>
</details>
...
4 cấp độ: <b style="color: var(--accent-primary);"><code>None</code></b>, <b style="color: var(--accent-primary);"><code>Metadata</code></b>, <b style="color: var(--accent-primary);"><code>Request</code></b>, và <b style="color: var(--accent-primary);"><code>RequestResponse</code></b>.
</div>
</details>
...
<code>Metadata</code> <b style="color: var(--accent-primary);">chỉ ghi lại thông tin định danh</b> (User, Verb, IP, Resource), còn <code>RequestResponse</code> <b style="color: var(--accent-primary);">ghi đầy đủ cả payload nội dung request và response body</b>.
</div>
</details>
...
Cờ <code>--audit-policy-file=/etc/kubernetes/audit/policy.yaml</code> và <code>--audit-log-path=/var/log/kubernetes/audit/audit.log</code>.
</div>
</details>
...
<code>apiVersion: audit.k8s.io/v1</code> (với <code>kind: Policy</code>).
</div>
</details>
...
Để <b style="color: var(--accent-primary);">bỏ qua các sự kiện rác có tần suất cực cao</b>, tránh làm phình to tràn ổ đĩa đệm.
</div>
</details>
...
<code>--audit-log-maxage</code> (số ngày tối đa), <code>--audit-log-maxbackup</code> (số file backup tối đa), và <code>--audit-log-maxsize</code> (dung lượng MB mỗi file).
</div>
</details>
...
Đường dẫn <code>/var/log/kubernetes/audit/audit.log</code>.
</div>
</details>
...
Để tiến trình <code>kube-apiserver</code> có <b style="color: var(--accent-primary);">quền ghi tệp nhật ký</b> ra ổ đĩa Host.
</div>
</details>
...
Công cụ <b style="color: var(--accent-primary);"><code>jq</code></b> (kết hợp với <code>grep</code>).
</div>
</details>
...
Mã phản hồi <b style="color: var(--accent-primary);"><code>403</code></b> (Forbidden).
</div>
</details>
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Phân biệt lượng dữ liệu ghi lại giữa 4 cấp độ Audit Levels: <code>None</code>, <code>Metadata</code>, <code>Request</code>, và <code>RequestResponse</code>?</span>
  </div>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>None</code>: <b style="color: var(--accent-primary);">Không ghi</b> log sự kiện.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>Metadata</code>: <b style="color: var(--accent-primary);">Chỉ ghi thông tin định danh</b> (User, Verb, Namespace, Resource, Timestamp, IP, ResponseStatus).</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>Request</code>: Ghi Metadata + <b style="color: var(--accent-primary);">Nội dung payload request body</b>.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>RequestResponse</code>: Ghi Metadata + Payload Request + <b style="color: var(--accent-primary);">Nội dung payload response body</b>.</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Không phân biệt được 4 cấp độ Audit Levels.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu đúng 2 cấp độ nhưng chưa làm rõ sự khác biệt payload body.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Phân tích chuẩn xác 100% chi tiết dữ liệu của 4 cấp độ Audit Levels.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Cấp độ nào chiếm dung lượng đĩa lớn nhất? — Cấp độ <b style="color: var(--accent-primary);"><code>RequestResponse</code></b>).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Cấu trúc một tệp chính sách <code>audit-policy.yaml</code> chuẩn CKS bắt buộc phải có những trường dữ liệu cơ bản nào?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);"><code>apiVersion: audit.k8s.io/v1</code>, <code>kind: Policy</code>, và khối <code>rules</code> chứa danh sách các quy tắc khai báo <code>level</code> đi kèm bộ lọc <code>resources</code>, <code>verbs</code> hoặc <code>users</code>.</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Viết sai apiVersion hoặc thiếu kind Policy.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu được khối rules nhưng thiếu apiVersion <code>audit.k8s.io/v1</code>.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Trình bày chuẩn xác cấu trúc tệp chính sách <code>audit-policy.yaml</code>.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Tại sao nên đặt quy tắc <code>level: None</code> ở đầu danh sách rules cho các request rác? — Để loại bỏ sớm các sự kiện tần suất cao trước khi đánh giá các quy tắc phía dưới).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Hai cờ câu lệnh bắt buộc phải bổ sung vào tệp Static Pod <code>kube-apiserver.yaml</code> để kích hoạt Audit Logging là gì?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>--audit-policy-file=/etc/kubernetes/audit/policy.yaml</code> (Chỉ định đường dẫn tệp chính sách).</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>--audit-log-path=/var/log/kubernetes/audit/audit.log</code> (Chỉ định đường dẫn tệp lưu nhật ký).</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Không nêu được 2 cờ apiserver.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu đúng 1 cờ audit-log-path nhưng thiếu audit-policy-file.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Trình bày chuẩn xác 2 cờ câu lệnh kích hoạt Audit Logging trên kube-apiserver.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Điều gì xảy ra nếu chỉ khai báo <code>--audit-log-path</code> mà quên cờ <code>--audit-policy-file</code>? — <code>kube-apiserver</code> bị crashloop không thể khởi động được).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Ba cờ xoay vòng log tự động trên <code>kube-apiserver</code> đóng vai trò gì trong việc bảo vệ hệ thống đĩa cứng của Host Node?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>--audit-log-maxage=30</code>: Tự động xóa tệp log cũ quá 30 ngày.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>--audit-log-maxbackup=10</code>: Giữ tối đa 10 tệp log nén sao lưu.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <code>--audit-log-maxsize=100</code>: Tự động xoay file log mới khi tệp hiện tại đạt dung lượng 100MB.</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Không biết các cờ xoay vòng log.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu được maxage và maxsize nhưng chưa rõ vai trò chống phình tràn ổ đĩa.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Phân tích chuẩn xác vai trò bảo vệ ổ đĩa Host của 3 cờ xoay vòng log Audit.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Nếu không cấu hình 3 cờ xoay vòng log này thì rủi ro gì xảy ra? — Tệp <code>audit.log</code> sẽ phình to liên tục làm tràn 100% dung lượng đĩa Host Node làm dừng toàn cụm).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Tại sao thư mục chứa tệp nhật ký Audit <code>/var/log/kubernetes/audit</code> bắt buộc phải được mount vào container <code>kube-apiserver</code> với cờ <code>readOnly: false</code>?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">Vì tiến trình <code>kube-apiserver</code> cần <b style="color: var(--accent-primary);">quyền ghi (Write permission)</b> dữ liệu nhật ký kiểm toán ra tệp tin nằm trên đĩa cứng ngoài Host Node. Nếu mount <code>readOnly: true</code>, apiserver sẽ bị từ chối quyền ghi log.</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Tưởng rằng mount readOnly: true.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu được cần quyền ghi nhưng chưa giải thích rõ mount hostPath volume.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Phân tích chuẩn xác lý do bắt buộc mount readOnly: false cho thư mục tệp log.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Thư mục tệp chính sách <code>/etc/kubernetes/audit</code> nên mount ở chế độ readOnly nào? — Mount ở chế độ <b style="color: var(--accent-primary);"><code>readOnly: true</code></b> để bảo vệ tệp policy không bị chỉnh sửa).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Cú pháp lệnh <code>jq</code> chuẩn để lọc ra tất cả các hành vi thao tác trên Secret bị từ chối <code>403 Forbidden</code> từ tệp <code>audit.log</code> là gì?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">grep "secrets" /var/log/kubernetes/audit/audit.log | jq -r 'select(.responseStatus.code==403) | [.stageTimestamp, .user.username, .verb, .objectRef.namespace, .objectRef.name, .sourceIPs[0]] | @tsv'</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">```</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Không biết lệnh jq lọc audit log.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu được grep secrets nhưng thiếu hàm select code==403 trong jq.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Trình bày chính xác 100% cú pháp <code>jq</code> trích xuất sự kiện 403 Forbidden trên Secret.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Trường <code>.sourceIPs[0]</code> trong log trích xuất thông tin gì? — Địa chỉ IP nguồn của kẻ thực hiện request).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Quy trình 4 bước để tái dựng lại kịch bản tấn công (Attack Reconstruction) từ tệp <code>audit.log</code> khi phát hiện một Secret bị lộ là gì?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <b style="color: var(--accent-primary);">Lọc tài nguyên</b>: <code>grep "secrets/db-pass"</code> lọc tất cả sự kiện liên quan tới Secret bị lộ.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <b style="color: var(--accent-primary);">Xác định thời điểm & User</b>: Tìm dòng log trích xuất <code>user.username</code> và <code>stageTimestamp</code>.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <b style="color: var(--accent-primary);">Trace ngược hành vi</b>: Lọc theo <code>user.username</code> đó để xem kẻ tấn công đã làm gì trước đó (như tạo Pod privileged hay sửa RoleBinding).</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <b style="color: var(--accent-primary);">Truy vết nguồn gốc IP</b>: Trích xuất <code>sourceIPs[0]</code> để xác định vị trí máy tấn công.</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Không nêu đủ 4 bước tái dựng kịch bản tấn công.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu được tìm user nhưng chưa rõ quy trình trace ngược các hành vi liên quan.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Phân tích thấu đáo quy trình 4 bước tái dựng kịch bản tấn công từ nhật ký Audit.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Nếu kẻ tấn công dùng ServiceAccount để truy cập thì thông tin User hiển thị thế nào? — Hiển thị dạng <code>system:serviceaccount:<namespace>:<sa-name></code>).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Sự khác biệt về mặt bản chất giữa <code>Audit Log</code> (K8s Control Plane) và <code>Container Log</code> (Application stdout) là gì?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <b style="color: var(--accent-primary);">Audit Log</b>: Ghi lại <b style="color: var(--accent-primary);">tất cả các request gửi tới API Server</b> (thao tác quản trị K8s đối tượng).</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• <b style="color: var(--accent-primary);">Container Log</b>: Ghi lại <b style="color: var(--accent-primary);">nhật ký xuất ra từ tiến trình ứng dụng</b> chạy bên trong container (như Nginx access log hay Java app log).</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Nhầm lẫn giữa Audit Log và Container Log.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu được 1 cái K8s 1 cái ứng dụng nhưng chưa phân biệt API Server request vs application stdout.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Phân tích chuẩn xác sự khác biệt về kiến trúc và mục đích của Audit Log vs Container Log.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Lệnh <code>kubectl logs</code> được dùng để xem loại log nào? — Dùng để xem <b style="color: var(--accent-primary);">Container Log</b> (application stdout), không xem được Audit Log).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Bốn giai đoạn xử lý yêu cầu (Audit Stages: <code>RequestReceived</code>, <code>ResponseStarted</code>, <code>ResponseComplete</code>, <code>Panic</code>) giai đoạn nào thường được dùng để ghi log nhất?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">Giai đoạn <b style="color: var(--accent-primary);"><code>ResponseComplete</code></b>! Vì ở giai đoạn này request đã xử lý xong, tệp log chứa đầy đủ cả thông tin request lẫn kết quả mã phản hồi thành công hay thất bại (<code>responseStatus.code</code>).</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 0đ: Không biết các Audit Stages.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu được ResponseComplete nhưng chưa giải thích vì sao (chứa mã phản hồi thành công/thất bại).</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Phân tích chuẩn xác lý do chọn stage <code>ResponseComplete</code> để ghi log kiểm toán.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Khi nào thì stage <code>Panic</code> xuất hiện? — Khi API Server gặp sự cố sập nội bộ nghiêm trọng khi đang xử lý request).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Cú pháp YAML chuẩn của tệp <code>audit-policy.yaml</code> ghi <code>RequestResponse</code> cho <code>secrets</code> và <code>Metadata</code> cho các tài nguyên còn lại CKS là gì?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">`</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">apiVersion: audit.k8s.io/v1</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">kind: Policy</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">rules:</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);"># Ghi RequestResponse cho Secret:</div>
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">resources: ["secrets"]</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);"># Ghi Metadata cho toàn bộ các tài nguyên còn lại:</div>
  ...
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 1đ: Nêu đúng level RequestResponse nhưng thiếu kind Policy hoặc rule Metadata mặc định.</div>
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Viết chuẩn xác 100% tệp <code>audit-policy.yaml</code> CKS.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Nếu bỏ qua dòng <code>- level: Metadata</code> ở cuối thì các tài nguyên khác có được ghi log không? — KHÔNG! Mặc định sẽ bị bỏ qua (mức <code>None</code>)).
...
<summary class="qa-summary">
  <div class="qa-summary-left">
    ...
    <span>Bộ 4 quy tắc vàng để làm chủ Audit Logging & Incident Response CKS là gì?</span>
  </div>
  <span class="qa-chevron">
    <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
  </span>
...
<div class="qa-answer">
  ...
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• Bật Audit Logging trên 100% cụm Production với tệp <code>audit-policy.yaml</code> chuẩn <code>apiVersion: audit.k8s.io/v1</code>.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• Ghi cấp độ <code>RequestResponse</code> cho các tài nguyên nhạy cảm (<code>secrets</code>) và <code>Metadata</code> cho phần còn lại.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• Cấu hình xoay vòng log (<code>maxage</code>, <code>maxbackup</code>, <code>maxsize</code>) để bảo vệ dung lượng đĩa cứng Host Node.</div>
  <div style="margin: 0.35rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• Thành thục kỹ năng dùng <code>grep</code> và <code>jq</code> trên tệp <code>audit.log</code> để tái dựng lại kịch bản tấn công trong vòng 5 phút.</div>
  <div style="margin-top: 0.75rem;"><b style="color: var(--accent-primary);">Tiêu chí chấm điểm &amp; Phân tầng năng lực:</b></div>
  ...
  <div style="margin: 0.25rem 0; padding-left: 1rem; border-left: 2px solid var(--accent-primary);">• 3đ: Trình bày tự tin, mạch lạc bộ 4 quy tắc vàng Audit Logging CKS.</div>
  <div style="margin-top: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(var(--accent-primary-rgb, 59, 130, 246), 0.08); border-radius: 4px;"><b style="color: var(--accent-primary);">Câu hỏi mở rộng / Đào sâu:</b> (Mục tiêu tiếp theo của bạn trong Buổi 64 là gì? — Học về <code>Giám sát và Phát hiện Thời gian chạy bằng Falco: Syscall Inspection & Runtime Threat Detection</code>).

---

## V3. Câu chốt để nói khi phỏng vấn

1. <b style="color: var(--accent-primary);">"Audit Logging là công cụ duy nhất ghi lại 100% các hành vi tương tác với API Server để phục vụ truy vết sự cố."</b>
2. <b style="color: var(--accent-primary);">"Phân bổ hợp lý 4 cấp độ Audit Levels (<code>RequestResponse</code> cho Secret, <code>Metadata</code> cho tài nguyên chung, <code>None</code> cho rác)."</b>
3. <b style="color: var(--accent-primary);">"Bảo vệ dung lượng đĩa cứng Host Node bằng bộ 3 cờ xoay vòng log (<code>maxage</code>, <code>maxbackup</code>, <code>maxsize</code>)."</b>
4. <b style="color: var(--accent-primary);">"Tái dựng 100% chuỗi hành vi tấn công từ tệp <code>audit.log</code> bằng kỹ thuật trích xuất JSON dữ liệu qua <code>jq</code>."</b>
...
Thiết lập **Kubernetes Audit Logging** với chính sách phân tầng thông minh (Metadata, Request, RequestResponse) và cơ chế lưu trữ an toàn giúp đội ngũ Security / SRE nắm trọn bức tranh vận hành, phát hiện sớm các cuộc tấn công leo thang đặc quyền và đảm bảo tuân thủ các tiêu chuẩn an ninh quốc tế (PCI-DSS, SOC 2).

> [!TIP]
> **BÀI HỌC TIẾP THEO:**
> Trong **[[Bài 19] Falco & Giám Sát An Ninh Thời Gian Chạy (Runtime Security & Threat Detection)](cks-19-19-falco-va-phat-hien-thoi-gian-chay.html)**, chúng ta sẽ bước vào lớp phòng thủ Runtime Security: Khám phá kiến trúc eBPF/Kernel Module của Falco, viết quy tắc phát hiện đe dọa (Falco Rules) và tích hợp cảnh báo thời gian thực khi có hành vi vi phạm.
{% endraw %}
