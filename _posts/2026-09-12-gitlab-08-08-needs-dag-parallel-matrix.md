...
$$	ext{Pipeline Duration} = \max_{	ext{Path } k} \left( \sum_{J_i \in 	ext{Path } k} 	ext{Duration}(J_i) ight)$$
...
2. **Parallel Matrix (`parallel: matrix`)**:
   ...
   - Ví dụ: 3 phiên bản Node.js $	imes$ 2 hệ điều hành $ightarrow$ Tự động sinh ra $6$ job con độc lập.
...
graph TD
    ...
    W1["Tại sao Pipeline bị từ chối? GitLab Parser phát hiện đồ thị có vòng lặp"]
    W2["Tại sao có vòng lặp? Job A phụ thuộc Job B, và Job B phụ thuộc Job A"]
    W3["Tại sao lại khai báo phụ thuộc chéo? Hai kỹ sư sửa hai file YAML con được ghép qua include:"]
    W4["Tại sao không phát hiện sớm? Không chạy công cụ CI Lint trước khi commit"]
    W5["Giải pháp cốt lõi: Sử dụng quy tắc Topological Sort và kiểm tra CI Lint API trước khi push"]
...
#### Cạm bẫy 1: Lỗi vòng lặp đồ thị phụ thuộc (Cyclic Dependency Error)
- **Hiện tượng**: Khi push code, GitLab từ chối tạo pipeline và báo lỗi `Pipeline failed to create: needs graph contains cycles`.
- **Nguyên nhân tầng sâu**: Khai báo phụ thuộc luẩn quẩn: Job A needs Job B, Job B needs Job C, và Job C lại needs Job A. Đồ thị DAG không thể thực hiện thuật toán sắp xếp tô-pô (Topological Sort).
- **Cách gỡ rối**: Vẽ lại sơ đồ luồng dữ liệu một chiều từ upstream sang downstream; tách các job phụ thuộc dữ liệu thành các node riêng biệt.

#### Cạm bẫy 2: Lỗi thiếu `optional: true` khi Job cha bị bỏ qua bởi `rules:`
- **Hiện tượng**: Pipeline trên nhánh `develop` chạy bình thường, nhưng khi chạy trên nhánh `feature` thì báo lỗi: `job_b needs job_a, but job_a is not in the pipeline`.
- **Nguyên nhân**: `job_a` có điều kiện `rules: - if: '$CI_COMMIT_BRANCH == "develop"'` nên trên feature branch nó không được tạo ra. `job_b` khai báo `needs: [job_a]` nhưng thiếu cờ `optional: true`.
- **Biện pháp**: Đổi thành `needs: [{ job: job_a, optional: true }]`.

#### Cạm bẫy 3: Cạn kiệt hàng đợi Runner do bùng nổ Parallel Matrix
- **Hiện tượng**: Kỹ sư khai báo ma trận kiểm thử $5 	ext{ OS} 	imes 4 	ext{ Versions} 	imes 3 	ext{ Databases} = 60 	ext{ jobs}$. Toàn bộ các pipeline khác của công ty bị kẹt ở trạng thái Pending nhiều giờ.
- **Nguyên nhân**: Số lượng job bùng nổ vượt xa dung lượng Runner Pool (ví dụ Runner chỉ có 10 concurrent slots).
- **Biện pháp**: Tối ưu hóa ma trận: Chỉ chạy full matrix trên `main` hoặc scheduled nightly build; trên Merge Request chỉ chạy 1-2 phiên bản đại diện.

#### Cạm bẫy 4: Mất mát Artifacts do quên khai báo `artifacts: true` trong `needs:`
- **Hiện tượng**: `job_deploy` cần file `.bin` từ `job_build`. Nhưng khi vào script chạy thì báo `file not found`.
- **Nguyên nhân**: Khai báo `needs: [{ job: job_build, artifacts: false }]` hoặc chỉ định `dependencies: []` làm ghi đè cơ chế tải artifact của needs.
- **Biện pháp**: Luôn bật `artifacts: true` khi job sau cần tiêu thụ sản phẩm của job trước.

#### Cạm bẫy 5: Ghi đè báo cáo kiểm thử khi gộp kết quả Parallel Sharding
- **Hiện tượng**: Chạy 4 shards test nhưng trên giao diện GitLab MR chỉ hiển thị báo cáo của 1 shard.
- **Nguyên nhân**: Tất cả các shard đều ghi kết quả ra cùng một tên file tĩnh `junit.xml`, khi upload lên server bị ghi đè hoặc trùng mã testsuite.
- **Biện pháp**: Đặt tên file động theo index: `junit-${CI_NODE_INDEX}.xml`.
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Kiến trúc DAG (Directed Acyclic Graph)</strong> là mô hình điều phối thực thi phi tuyến tính, trong đó các Job được liên kết với nhau bằng các cạnh phụ thuộc trực tiếp một chiều không chu trình (thông qua từ khóa <code>needs:</code>).</p>
    <p><strong>Lý do tối ưu thời gian</strong>: Trong mô hình Stage truyền thống, tồn tại "Rào cản giai đoạn" (Stage Barrier) khiến các job thuộc stage sau phải chờ <em>toàn bộ</em> các job của stage trước hoàn tất (kể cả job chậm nhất). Với DAG, một job có thể bắt đầu ngay lập tức khi các phụ thuộc trực tiếp của riêng nó hoàn thành, tách biệt các luồng công việc độc lập (như Frontend vs Backend) và rút ngắn thời gian phản hồi (Fast Feedback) cho lập trình viên.</p>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Job chạy theo Stage thông thường</strong>: Mặc định sẽ tải về <em>toàn bộ artifacts</em> được tạo ra bởi tất cả các job thuộc tất cả các stage trước đó trong pipeline.</p>
    <p><strong>Job sử dụng <code>needs: [job_a, job_b]</code></strong>: Mặc định <em>chỉ tải duy nhất artifacts</em> của các job được liệt kê trong danh sách <code>needs</code>. Nếu muốn tắt hoàn toàn việc tải artifact để tiết kiệm băng thông mạng, ta có thể khai báo tường minh: <code>needs: [{ job: job_a, artifacts: false }]</code>.</p>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Khi nào bắt buộc dùng</strong>: Khi job mục tiêu nằm trong <code>needs</code> có chứa điều kiện <code>rules:</code> và có thể bị loại bỏ (không được tạo) trong một số tình huống pipeline (ví dụ: chỉ chạy trên branch <code>main</code> hoặc khi commit chứa tag).</p>
    <p><strong>Lỗi nếu không khai báo</strong>: Khi pipeline khởi tạo trên branch không thỏa mãn rules của job mục tiêu, GitLab Server sẽ từ chối tạo pipeline với thông báo lỗi: <em>"job_b needs job_a, but job_a is not in the pipeline"</em>.</p>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong><code>parallel: N</code> (Parallel Sharding)</strong>:</p>
    <ul>
      <li><em>Bản chất</em>: Nhân bản 1 job thành $N$ bản sao giống nhau, tự động tiêm biến <code>CI_NODE_INDEX</code> và <code>CI_NODE_TOTAL</code>.</li>
      <li><em>Ứng dụng</em>: Chia đều danh sách file test nặng (E2E Test, Unit Test) để chạy song song trên nhiều Runner, giảm thời gian kiểm thử tuyến tính.</li>
    </ul>
    <p><strong><code>parallel: matrix</code> (Parallel Matrix)</strong>:</p>
    <ul>
      <li><em>Bản chất</em>: Sinh ra các job tương ứng với tích Descartes của các mảng biến số cấu hình.</li>
      <li><em>Ứng dụng</em>: Kiểm thử tính tương thích đa môi trường (ví dụ test thư viện trên Node 18, 20, 22 kết hợp trên OS Linux, Windows, macOS).</li>
    </ul>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p>Khai báo thuộc tính <strong><code>needs: []</code> (Mảng rỗng)</strong> trong cấu hình của Job. Khi đó, GitLab Runner sẽ xếp job vào hàng đợi thực thi ngay tại $t_0$ mà không cần chờ các job hoặc stage trước đó, đồng thời job này sẽ không tải bất kỳ artifact nào.</p>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Đường găng (Critical Path)</strong> là chuỗi các công việc nối tiếp nhau có quan hệ phụ thuộc mà tổng thời gian thực hiện của chúng là dài nhất trong toàn bộ đồ thị pipeline.</p>
    <p><strong>Công thức</strong>:</p>
    <p>$$	ext{Thời gian Pipeline} = \max_{	ext{Tất cả các luồng phụ thuộc}} \left( \sum 	ext{Thời gian các Job trên luồng} ight)$$</p>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Giải pháp</strong>: Đặt tên file artifact chứa biến <code>CI_NODE_INDEX</code> động trong script, ví dụ: <code>reports/junit-${CI_NODE_INDEX}.xml</code>. Sau đó trong phần <code>artifacts:reports:junit</code> sử dụng ký tự đại diện glob: <code>reports/junit-*.xml</code>.</p>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>KHÔNG THỂ</strong>. Mặc dù <code>needs:</code> cho phép phá vỡ rào cản tuần tự giữa các stage liền kề, nhưng GitLab vẫn bắt buộc job được trỏ tới trong <code>needs</code> phải thuộc về <strong>cùng stage hoặc các stage TRƯỚC ĐÓ</strong> theo thứ tự khai báo trong mảng <code>stages:</code>.</p>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Nguyên lý hoạt động</strong>: Cho phép một job trong Project A tải trực tiếp artifact từ một bản build thành công của Project B thông qua khai báo:</p>
  ...
  <span class="pi">-</span> <span class="na">project</span><span class="pi">:</span> <span class="s1">'</span><span class="s">group/backend-api'</span>
    ...
    <p><strong>Yêu cầu quyền hạn</strong>: User kích hoạt pipeline hoặc Token CI phải có tối thiểu quyền <strong>Developer</strong> hoặc <strong>Reporter</strong> trên project nguồn, và tính năng <em>CI/CD Job Token Access</em> giữa hai dự án phải được cấp phép (Allowlisted).</p>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Trường hợp lỗi</strong>: Khi tồn tại một chu trình khép kín trong đồ thị phụ thuộc (ví dụ $A ightarrow B ightarrow C ightarrow A$).</p>
    <p><strong>Thuật toán phát hiện</strong>: GitLab CI Engine sử dụng thuật toán <strong>Topological Sorting (Kahn's Algorithm hoặc Tarjan's Strongly Connected Components)</strong> để duyệt đồ thị. Nếu sau khi loại bỏ các node có bậc vào bằng 0 mà đồ thị vẫn còn đỉnh, thuật toán kết luận tồn tại chu trình (Cycle) và lập tức từ chối khởi tạo pipeline.</p>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong>Chiến lược thiết kế tối ưu:</strong></p>
    <ol>
      <li><strong>Sử dụng <code>rules:changes</code></strong>: Chỉ tạo ra các nhánh DAG tương ứng với phần code bị thay đổi trong commit/MR.</li>
      <li><strong>Tách riêng 3 luồng DAG độc lập</strong>:
        <ul>
          <li>Luồng Frontend: <code>build_fe</code> $ightarrow$ <code>test_fe</code> $ightarrow$ <code>deploy_cdn</code>.</li>
          <li>Luồng Backend: <code>build_be</code> $ightarrow$ <code>test_be_matrix</code> $ightarrow$ <code>deploy_k8s</code>.</li>
          <li>Luồng Mobile: <code>build_apk</code> $ightarrow$ <code>test_mobile</code> $ightarrow$ <code>upload_firebase</code>.</li>
        </ul>
      </li>
      <li><strong>Cô lập Artifacts</strong>: Dùng <code>needs: [{ job: ..., artifacts: false }]</code> ở các bước không cần binary để tránh nghẽn I/O.</li>
    </ol>
  </div>
</details>
