...
unit_and_integration_tests:
  ...
  script:
    ...
    - pytest tests/         --splits=${CI_NODE_TOTAL}         --group=${CI_NODE_INDEX}         --junitxml="junit-${CI_NODE_INDEX}.xml"         --cov=src         --cov-report=xml:coverage-${CI_NODE_INDEX}.xml         --cov-report=term
  artifacts:
    reports:
      junit: "junit-*.xml"
      coverage_report:
...
<details class="qa-card">
  ...
  <div class="qa-body">
    <p><strong><code>tox</code> chạy tuần tự</strong>: Chạy lần lượt các môi trường test (Python 3.10 $ightarrow$ 3.11 $ightarrow$ 3.12) trên cùng một máy chủ đơn lẻ. Tổng thời gian bằng tổng thời gian của cả 3 phiên bản cộng lại (ví dụ $5m 	imes 3 = 15m$).</p>
    <p><strong><code>parallel: matrix</code> của GitLab CI</strong>: Tự động nhân bản thành 3 Job độc lập chạy <strong>song song trên 3 Runner khác nhau</strong> cùng lúc. Tổng thời gian hoàn thành chỉ bằng đúng 1 phiên bản chạy ($5m$), rút ngắn 66% thời gian chờ đợi.</p>
  </div>
</details>
