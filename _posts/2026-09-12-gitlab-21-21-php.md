...
description: "Xây dựng Pipeline chuẩn mực cho PHP & Laravel Framework: Tối ưu hóa Composer cache, kiểm tra tĩnh với PHPStan/Psalm, chạy PHPUnit và triển khai bảo mật zero-downtime."
keywords:
  - gitlab ci php
  - gitlab composer cache
  - gitlab phpunit phpstan
  - gitlab laravel pipeline
---
...
unit_and_feature_tests:
  ...
  script:
    ...
    - php -d extension=pcov.so -d pcov.enabled=1 ./vendor/bin/phpunit \
        --log-junit junit.xml \
...
<span class="badge badge--rose">🕒 06:10 AM</span> Pipeline kiểm thử tự động của dự án PHP Laravel với hơn 400 bài test tích hợp bị timeout sau 15 phút chạy trên GitLab Runner. Khi kiểm tra log, mỗi test case đơn giản mất tới 1.2 đến 2.5 giây để hoàn thành, khiến hàng đợi Merge Request bị ứ đọng nghiêm trọng.

### Hậu Quả & Log Lỗi Thực Tế:

```text

Runner bị hủy do vượt quá thời gian tối đa (`Job execution exceeded timeout of 15m0s`):

$ php ./vendor/bin/phpunit --coverage-text
PHPUnit 11.2.5 by Sebastian Bergmann and contributors.
Runtime:       PHP 8.3.8 with Xdebug 3.3.2
Configuration: /builds/group/laravel-app/phpunit.xml
...............................................................  63 / 412 ( 15%)
............................................................... 126 / 412 ( 30%)
...
1. <span class="badge badge--primary">Why 1</span> **Tại sao Job PHPUnit chạy quá 15 phút và bị timeout?** &rarr; Tốc độ thực thi của từng bài kiểm tra bị suy giảm nghiêm trọng khi xuất báo cáo độ phủ mã nguồn (Code Coverage).
2. <span class="badge badge--primary">Why 2</span> **Tại sao việc đo Code Coverage lại chạy chậm bất thường?** &rarr; Trình chạy kiểm thử đang sử dụng extension `Xdebug 3.x` để theo dõi luồng thực thi của mã nguồn PHP.
3. <span class="badge badge--primary">Why 3</span> **Tại sao Xdebug lại làm giảm hiệu năng tới 500%?** &rarr; Xdebug là công cụ Interactive Debugger, được thiết kế để bắt từng hook Opcode chi tiết trong Zend Engine, tạo ra overhead cực kỳ lớn trong môi trường máy chủ CI.
4. <span class="badge badge--primary">Why 4</span> **Tại sao lại dùng Xdebug trên môi trường CI?** &rarr; Kỹ sư cài đặt image PHP chung từ local development hoặc dùng lệnh `pecl install xdebug` theo thói quen cũ mà không tối ưu cho pipeline.
5. <span class="badge badge--emerald">Root Cause Remedy</span> **Giải pháp triệt để**: Vô hiệu hóa hoàn toàn Xdebug trong CI và chuyển sang cài đặt C-Extension `PCOV` (`pecl install pcov` hoặc qua FrankenPHP Alpine image) kết hợp lệnh `php -d extension=pcov.so -d pcov.enabled=1 ./vendor/bin/phpunit`.
...
<details class="qa-card">
  ...
  <div class="qa-answer">
    ...
    <ul>
      ...
      <li>Chạy siêu tốc trong vòng vài giây tại t0, bắt được 80% lỗi sai của lập trình viên trước khi bất kỳ bài Unit Test nào bắt đầu chạy.</li>
    </ul>
  </div>
</details>
...
<details class="qa-card">
  ...
  <div class="qa-answer">
    ...
    <ol>
      <li><strong>Stage Analyze</strong>: Chạy song song <code>phpstan</code> và <code>security-checker</code> kiểm tra lỗ hổng bảo mật của Composer packages tại t0.</li>
