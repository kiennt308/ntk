...
### 4.1. Sự Cố Thực Tế: Runner Thất Bại Do Package Registry Hết Dung Lượng (Disk Full)

> **Bối Cảnh**: Một công ty thương mại điện tử triển khai pipeline CI/CD đẩy Docker Image trên mỗi commit nhánh `feature/*`. Sau 3 tháng, hệ thống Harbor/Artifactory báo lỗi `HTTP 507 Insufficient Storage` làm tê liệt toàn bộ 200 dự án CI/CD của toàn công ty.

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    PHÂN TÍCH NGUYÊN NHÂN GỐC RỄ (5-WHYS)                 │
├─────────────────────────────────────────────────────────────────────────┤
│ 1. Tại sao các Job CI bị lỗi không thể đẩy Artifact/Image?             │
│    -> Máy chủ Registry báo lỗi HTTP 507 - Ổ đĩa lưu trữ đạt 100%.       │
│                                                                         │
│ 2. Tại sao ổ đĩa Registry lại đầy nhanh chóng?                          │
│    -> Có hơn 45.000 image tags của các commit nhánh phụ không được dọn. │
│                                                                         │
│ 3. Tại sao các image nhánh phụ lại được lưu vĩnh viễn?                 │
│    -> Pipeline tự động gắn tag SHA và không cấu hình Retention Policy.  │
│                                                                         │
│ 4. Tại sao cấu hình Retention Policy lại chưa được áp dụng?            │
│    -> Thiếu quy định phân tách giữa Release Tag bất biến và Snapshot.   │
│                                                                         │
│ 5. NGUYÊN NHÂN CỐT LÕI (Root Cause):                                   │
│    -> Không cấu hình tự động dọn rác (Garbage Collection Cron) và thiếu │
│       chính sách Tag Retention theo ngày cho nhánh Non-Protected.       │
└─────────────────────────────────────────────────────────────────────────┘
```

### 4.2. Giải Pháp Khắc Phục Triệt Để

1. **Thiết lập Tag Retention Rules trên Harbor / GitLab Registry**:
   - Nhánh `feature/*`: Chỉ giữ lại 5 tags mới nhất hoặc tự động xóa sau **3 ngày**.
   - Nhánh `main` / `tags`: Giữ vĩnh viễn (**Keep Forever**).
2. **Kích hoạt Garbage Collection định kỳ**: Chạy lệnh thu hồi dung lượng đĩa (GC dry-run / delete untagged manifests) vào mỗi 02:00 AM Chủ Nhật.
  ...
  script:
    - |
      for file in dist/*; do
        ...
        curl --header "JOB-TOKEN: ${CI_JOB_TOKEN}"              --upload-file "$file"              "${PACKAGE_REGISTRY_URL}/${filename}"
      done
