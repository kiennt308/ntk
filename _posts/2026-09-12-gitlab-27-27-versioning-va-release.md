...
### 4.1. Sự Cố Thực Tế: Vòng Lặp Vô Tận (Infinite Pipeline Recursion) Khi Bot Push Commit Release

> **Bối Cảnh**: Nhóm kỹ sư cấu hình Semantic-Release để tự động commit tệp `CHANGELOG.md` mới lên nhánh `main`. Ngay sau khi release thành công, commit mới của bot lại kích hoạt một pipeline mới, pipeline này lại tiếp tục chạy release, sinh ra hơn 200 bản release rác trong vòng 30 phút và làm tê liệt toàn bộ Runner pool.

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    PHÂN TÍCH NGUYÊN NHÂN GỐC RỄ (5-WHYS)                 │
├─────────────────────────────────────────────────────────────────────────┤
│ 1. Tại sao có hàng trăm Pipeline liên tục tự kích hoạt?                 │
│    -> Nhánh main liên tục nhận được các commit mới từ Release Bot.      │
│                                                                         │
│ 2. Tại sao Release Bot lại push commit mới?                            │
│    -> Bot cần cập nhật CHANGELOG.md vào repository sau khi tạo version. │
│                                                                         │
│ 3. Tại sao GitLab CI lại kích hoạt pipeline cho commit của Bot?         │
│    -> GitLab coi mọi commit push lên branch main là sự kiện hợp lệ.    │
│                                                                         │
│ 4. Tại sao Git commit của Bot không bị bỏ qua?                         │
│    -> Thông điệp commit của Bot thiếu cú pháp chỉ thị bỏ qua CI.        │
│                                                                         │
│ 5. NGUYÊN NHÂN CỐT LÕI (Root Cause):                                   │
│    -> Cấu hình @semantic-release/git không chèn chuỗi [skip ci] hoặc   │
│       [ci skip] vào trường commit message mẫu.                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
> [!NOTE]
> **Check-point Lab 27**: Ba lần commit tự động nâng phiên bản chính xác theo thứ tự `v1.1.0` -> `v1.1.1` -> `v2.0.0`, CHANGELOG được cập nhật đầy đủ và không xảy ra hiện tượng lặp vô tận pipeline nhờ cờ `[skip ci]`.
...
<details class="qa-card">
  ...
  <div class="qa-body">
    ...
    <ul>
      <li><code>fix:</code> hoặc <code>perf:</code> -> Nâng <strong>PATCH</strong> (Sửa lỗi, tối ưu hiệu năng).</li>
      <li><code>feat:</code> -> Nâng <strong>MINOR</strong> (Bổ sung tính năng mới).</li>
      <li>Bất kỳ commit nào có ký tự <code>!</code> sau type (ví dụ: <code>feat!:</code>) hoặc có dòng <code>BREAKING CHANGE:</code> trong phần body/footer -> Nâng <strong>MAJOR</strong>.</li>
      <li><code>docs:</code>, <code>style:</code>, <code>refactor:</code>, <code>test:</code>, <code>chore:</code> -> <strong>Không phát hành phiên bản mới</strong>.</li>
    </ul>
  </div>
</details>
