...
1. **Facts là Auto-discovery:** Module `setup` tự động quét hiện trạng hệ thống và đóng gói vào không gian tên `ansible_facts`.
2. **Luôn dùng cú pháp chuẩn `ansible_facts.<key>`:** Tránh dùng cú pháp phẳng cũ `ansible_<key>` để ngăn ngừa xung đột biến.
3. **Thư mục Custom Facts chuẩn:** Luôn đặt tại `/etc/ansible/facts.d/*.fact` trên máy đích (hỗ trợ INI, JSON và Script `0755`).
4. **Bắt buộc Reload Facts in-flight:** Gọi `ansible.builtin.setup: filter=ansible_local` ngay sau khi tạo file fact mới.
5. **Tối ưu tốc độ với Subset & Cache:** Dùng `gather_subset` và bật `fact_caching = jsonfile` để tăng tốc độ thực thi Playbook.

```mermaid
mindmap
  root((Facts & Custom Facts Mastery))
    Default Facts (setup)
      Auto-discovery Kernel/Hardware/OS
      Standard namespace ansible_facts
      gather_subset speed optimization
    Custom Facts (Local Facts)
      Path /etc/ansible/facts.d/*.fact
      Static INI & JSON
      Dynamic executable script 0755
      Namespace ansible_facts.ansible_local
    Performance & Operations
      Fact Caching: jsonfile & Redis
      In-flight reload setup filter
      Idempotency changed=0 run 2
      docker exec ground truth
```

> [!TIP]
> **BÀI HỌC TIẾP THEO:** [Bài 09: Điều Kiện & Rẽ Nhánh Logic (Conditionals With When, Changed_when & Failed_when)](ansible-09-09-conditionals-when.html)
{% endraw %}
