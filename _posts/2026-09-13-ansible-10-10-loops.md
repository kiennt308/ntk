...
1. **Luôn dùng từ khóa `loop:`:** Thay thế hoàn toàn cú pháp cũ `with_items` bằng `loop:`.
2. **Cấm dùng `loop:` cho Package:** Cài đặt gói phần mềm bằng danh sách mảng trong 1 transaction để tăng tốc 10x.
3. **Bảo vệ Secret với `label`:** Luôn cấu hình `loop_control.label` để ẩn thông tin nhạy cảm khỏi log CI/CD.
4. **Tránh xung đột với `loop_var`:** Đổi tên biến lặp khi xử lý các vòng lặp lồng nhau hoặc include task.
5. **Khai thác mảng `register.results`:** Toàn bộ dữ liệu của từng phần tử được lưu trong mảng `results` để phục vụ audit và xử lý tiếp theo.
...
mindmap
  root((Loops Mastery))
    Core Syntax
      ...
      List of Hashes Dicts
      dict2items for maps
    loop_control
      loop_var custom naming
      label secret hiding
      pause rate limiting
      index_var 0-indexed count
    Performance & Safety
      Package Batching NO LOOP
      until retries delay
      Item-by-item when evaluation
    Verification & Results
      register.results array
      Two-run test changed=0
      docker exec batch truth
```

> [!TIP]
> **BÀI HỌC TIẾP THEO:** [Bài 11: Handlers & Notify: Cơ Chế Kích Hoạt Sự Kiện, Flush_handlers & Tối Ưu Reload Dịch Vụ](ansible-11-11-handlers-notify.html)
{% endraw %}
