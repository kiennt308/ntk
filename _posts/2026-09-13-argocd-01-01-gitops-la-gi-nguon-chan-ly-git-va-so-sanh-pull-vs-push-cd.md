...
1. **Tại sao phiên bản mới v2.0 bị mất trên Production?** $\rightarrow$ Vì Argo CD đã ghi đè lại mã nguồn v1.0 từ Git.
2. **Tại sao Argo CD lại ghi đè?** $\rightarrow$ Vì kỹ sư bấm nút "Sync" trong khi Git vẫn lưu v1.0.
3. **Tại sao Git lại chỉ có v1.0?** $\rightarrow$ Vì kỹ sư sửa trực tiếp bằng lệnh `kubectl set image` thay vì tạo commit trên Git.
4. **Tại sao kỹ sư lại sửa bằng kubectl?** $\rightarrow$ Do thói quen xử lý sự cố khẩn cấp (Hotfix) kiểu cũ mà không tuân thủ quy trình GitOps.
5. **Bài học rút ra:** Mọi thay đổi không qua Git đều bị coi là "sai lệch bất hợp pháp" (Illegal Drift) và sẽ bị triệt tiêu!

---

## 7. Hướng Dẫn Thực Hành CLI: Kiểm Chứng Cơ Chế Phát Hiện Drift

Hãy tự tay thực hành kiểm tra khả năng phát hiện Drift của Argo CD bằng các câu lệnh sau:
...
### Câu 1: Tại sao GitOps yêu cầu hệ thống phải được mô tả bằng Declarative thay vì Imperative?
- **Đáp án:** Vì mô hình Declarative mô tả "kết quả mong muốn cuối cùng" (What you want) độc lập với trạng thái hiện tại. Điều này cho phép hệ thống tự động tính toán khoảng cách (Diff) và hội tụ trạng thái mà không gây ra lỗi trùng lặp khi chạy lại nhiều lần (Idempotency).

### Câu 2: Trong mô hình Pull-based GitOps, nếu Git Server (GitHub/GitLab) bị sập trong 1 giờ thì ứng dụng trên Kubernetes có bị dừng không?
- **Đáp án:** **Không!** Các Pods và dịch vụ trên Kubernetes vẫn tiếp tục hoạt động bình thường. Trong thời gian Git sập, chỉ có tính năng đồng bộ phiên bản mới bị tạm ngưng; hệ thống hiện tại hoàn toàn không bị ảnh hưởng.

### Câu 3: Làm thế nào để xử lý sự cố khẩn cấp (Hotfix) trên Production theo đúng chuẩn GitOps?
- **Đáp án:** Kỹ sư tạo một nhánh hotfix trên Git, commit thay đổi $\rightarrow$ Tạo Pull Request $\rightarrow$ Duyệt khẩn cấp (Emergency Approval) $\rightarrow$ Merge vào nhánh chính $\rightarrow$ Argo CD tự động kéo mã nguồn mới về triển khai trong vài giây. Lịch sử sửa lỗi được lưu vết 100% trên Git.

### Câu 4: Sự khác nhau cơ bản giữa GitOps và Infrastructure as Code (IaC như Terraform) là gì?
- **Đáp án:** IaC (Terraform) thường chạy theo mô hình Push (khi kỹ sư chạy `terraform apply`). Nếu ai đó sửa thủ công trên Cloud Console sau đó, Terraform không tự sửa lại nếu không có ai chạy lại lệnh. GitOps bổ sung thêm **Reconciliation Loop chạy liên tục 24/7** để tự động phát hiện và triệt tiêu sai lệch mà không cần sự can thiệp của con người.

### Câu 5: Cần lưu ý gì khi lưu trữ Secrets (Mật khẩu DB, API Keys) trong kho GitOps?
- **Đáp án:** Tuyệt đối không lưu Plaintext Secret lên Git. Bắt buộc phải sử dụng các giải pháp mã hóa an toàn như **Bitnami Sealed Secrets**, **External Secrets Operator (kết nối HashiCorp Vault / AWS Secrets Manager)**, hoặc **SOPS**.

### Câu 6: Thuật ngữ "Single Source of Truth" trong GitOps có ý nghĩa gì đối với việc kiểm toán (Audit)?
- **Đáp án:** Nghĩa là mọi cấu hình đang chạy trên thực tế đều bắt buộc phải có nguồn gốc từ một commit cụ thể trên Git. Kiểm toán viên chỉ cần kiểm tra lịch sử Git Commit và Pull Request là có thể xác minh 100% ai đã thay đổi gì, vào thời điểm nào và đã qua những bước phê duyệt nào.

### Câu 7: Điều gì xảy ra nếu hai người cùng sửa một cấu hình ứng dụng trên Git cùng lúc?
- **Đáp án:** Git sẽ sử dụng cơ chế xử lý xung đột phân nhánh (Merge Conflict Resolution). Người thứ hai bắt buộc phải rebase hoặc merge nhánh mới nhất trước khi Pull Request được chấp thuận, đảm bảo tính nhất quán tuyệt đối trước khi đưa xuống cụm.

### Câu 8: Tại sao việc tách biệt App Code Repo và Config GitOps Repo là một kiến trúc chuẩn mực?
- **Đáp án:** (1) Giúp giảm thiểu số lần kích hoạt CI không cần thiết khi chỉ sửa config, (2) Cho phép phân quyền RBAC khác nhau (Developer có quyền push App Code nhưng chỉ có quyền tạo PR trên Config Repo), và (3) Ngăn chặn vòng lặp CI/CD vô tận khi CI commit image tag mới vào chính repo của nó.

### Câu 9: Tại sao mô hình Branch-per-Environment lại dễ dẫn đến tình trạng "Git Merge Hell"?
- **Đáp án:** Khi mỗi môi trường là một nhánh riêng (`dev`, `staging`, `prod`), các nhánh này theo thời gian sẽ có sự phân kỳ cấu hình (ví dụ nhánh dev thêm các biến debug nhưng không bao giờ đưa lên prod). Khi chạy `git merge` từ dev sang staging sang prod, các xung đột merge conflict sẽ xảy ra liên tục và làm tăng nguy cơ vô tình đưa cấu hình thử nghiệm lên Production.

### Câu 10: Cơ chế nào đảm bảo tính toàn vẹn của mã nguồn trên Git để tránh kẻ xấu chèn commit giả danh?
- **Đáp án:** Sử dụng **Chữ ký số GPG (GPG Commit Signing)** kết hợp với cơ chế **Signed Commits Enforcement** trên GitHub/GitLab. Argo CD có thể cấu hình tính năng `gpg.verification` để từ chối đồng bộ bất kỳ commit nào không có chữ ký GPG hợp lệ của các kỹ sư được ủy quyền.
