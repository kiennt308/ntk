...
```mermaid
flowchart LR
    subgraph IMP["MÔ HÌNH IMPERATIVE (MỆNH LỆNH)"]
        direction TB
        I1["Bước 1: Kiểm tra tài nguyên"] --> I2["Bước 2: Tạo nếu chưa có"]
        I2 --> I3["Bước 3: Cấu hình tham số"]
        I3 --> I4["Bước 4: Tự viết mã Rollback"]
    end

    subgraph DEC["MÔ HÌNH DECLARATIVE (KHAI BÁO)"]
        direction TB
        D1["Code HCL: Khai báo Desired State"] --> D2["Engine tự động tính toán Diff"]
        D2 --> D3["Reconciliation: Tự động đưa hệ thống về chuẩn"]
    end

    style IMP fill:#fff1f2,stroke:#e11d48,stroke-width:1px
    style DEC fill:#f0fdf4,stroke:#16a34a,stroke-width:1px
```
...
graph TD
    subgraph INPUTS["1. ĐẦU VÀO HỆ THỐNG"]
        HCL["Mã Nguồn HCL (.tf)<br/><b>Desired State: S_desired</b>"]
        STATE["Terraform State File (.tfstate)<br/><b>Recorded State: S_recorded</b>"]
        CLOUD["Hạ Tầng Thực Tế (Cloud API)<br/><b>Actual State: S_actual</b>"]
    end
    ...
    style INPUTS fill:#f8fafc,stroke:#94a3b8,stroke-width:1px
    style ENGINE fill:#eff6ff,stroke:#3b82f6,stroke-width:1px
    style OUTPUTS fill:#f0fdf4,stroke:#22c55e,stroke-width:1px
```
...
# ⚠️ Cơ sở dữ liệu bị Restart để hạ cấu hình, làm gián đoạn toàn bộ hệ thống thanh toán trong 15 phút!
```
...
flowchart TD
    ...
    style A fill:#fff1f2,stroke:#e11d48,stroke-width:1px
...
<details>
<summary><b>Câu 1: Bản chất khác biệt lớn nhất giữa Declarative IaC (Terraform) và Imperative IaC (Ansible/Bash) là gì?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 2: Trạng thái (State) trong Terraform đóng vai trò gì trong chu trình Reconcile Loop?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 3: Vì sao việc chạy `terraform apply -auto-approve` trên môi trường Production bị coi là phản mẫu (Anti-Pattern)?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 4: Tham số `prevent_destroy = true` trong khối `lifecycle` hoạt động như thế nào?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 5: Lệnh `terraform plan -refresh-only` khác gì so với `terraform plan` thông thường?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 6: Thuộc tính `bucket_key_enabled = true` trong cấu hình mã hóa S3 mang lại lợi ích gì?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 7: Khi nào nên sử dụng Pulumi thay vì Terraform trong các dự án Cloud Native?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 8: Hiện tượng "Index Shifting" trong Terraform là gì và công cụ nào giải quyết triệt để?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 9: Lệnh `terraform validate` kiểm tra những gì và nó có gọi tới Cloud API hay không?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
...
<details>
<summary><b>Câu 10: State Locking (Khóa trạng thái) hoạt động như thế nào khi nhiều kỹ sư cùng chạy Terraform?</b></summary>
<div style="margin-top: 1rem; padding: 1rem; background: var(--bg-surface); border-left: 3px solid var(--accent-primary); border-radius: var(--radius-sm);">
