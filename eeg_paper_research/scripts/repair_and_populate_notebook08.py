...
KW_DIR = os.path.join(BASE_DIR, r"literature\open_access_repository\kw08_transformers_crossmodal_affective")
NB_DIR = os.path.join(BASE_DIR, r"notebooklm_workspace\notebook_08_Transformers_Crossmodal_Affective_Computing")
KW_PDFS = os.path.join(KW_DIR, "pdfs")
NB_PDFS = os.path.join(NB_DIR, "pdfs")
TEMP_STAGING = os.path.join(BASE_DIR, "staging_unique_kw08")
...
def is_valid_pdf(file_path):
    ...
    try:
        ...
        if "cern" in first_page and "lhc" in first_page:
            return False
        keywords = [
            "eeg", "ecg", "eda", "biosignal", "physiological", "transformer", "cross-modal", "multimodal", 
            "attention", "crossmodal", "emotion", "affective", "bci", "brain", "self-attention", "fusion"
        ]
...
def main():
    print("=== STARTING NOTEBOOK 08 REPAIR & POPULATION (50 STRICTLY UNIQUE PAPERS) ===")
    
    unique_pool = []
    seen_hashes = set()
    seen_titles = set()
    
    # 1. First inspect existing PDFs in kw08
    existing_files = sorted([f for f in os.listdir(KW_PDFS) if f.endswith(".pdf")])
    print(f"Existing files in KW08: {len(existing_files)}")
        ...
        if is_valid_pdf(fpath):
            ...
            if not title:
                try:
                    ...
                    title = f"Transformer Crossmodal Biosignals Paper {fname}"
    ...
    if len(unique_pool) < 50:
        ...
        print(f"Need to download {needed} more unique papers for KW08...")
        
        openalex_queries = [
            "cross-modal transformer multimodal emotion recognition EEG",
            "crossmodal attention biosignals affective computing",
            "transformer EEG ECG multimodal emotion recognition",
            "multimodal bottleneck transformer physiological signals",
            "hierarchical transformer continuous emotion recognition EEG",
            "spatial-temporal transformer EEG affective computing",
            "cross-attention physiological time series emotion",
            "multimodal transformer wearable emotion recognition",
            "perceiver multimodal biosignals emotion recognition",
            "deep transformer cross-modal fusion EEG peripheral",
            "conformer EEG emotion recognition biosignals",
            "attention mechanism multimodal affective computing"
        ]
        arxiv_queries = [
            "cross-modal transformer EEG emotion",
            "multimodal transformer emotion recognition biosignals",
            "cross-attention EEG ECG emotion",
            "transformer physiological signals emotion",
            "multimodal cross-modal attention affective",
            "temporal transformer EEG affective computing"
        ]
    ...
    for i, p in enumerate(final_50, 1):
        ...
        note_content = generate_paper_notes(p, "OA_KW8", i)
        ...
        metadata_list.append({
            ...
            "file_name": fname,
            "md5_hash": p["hash"],
            "cluster": "KW08_Transformers_Crossmodal_Affective_Computing"
        })
    ...
    # Write JSON metadata
    json_content = {
        "cluster_id": "notebook_08",
        "cluster_name": "Transformers & Crossmodal Affective Computing",
        "description": "Curated collection of 50 verified full-text Open Access research papers (2022-2026) on Transformer architectures, cross-modal attention, spatiotemporal transformers, and multimodal fusion for EEG and biosignal affective computing.",
    ...
    prompts_md = f"""# NotebookLM Workspace: Cluster 08 - Transformers & Crossmodal Affective Computing
...
"""
    ...
    print("=== NOTEBOOK 08 REPAIR & POPULATION COMPLETED SUCCESSFULLY! ===")
